...
\begin{document}
  ...
  \paragraph{}
  Ultimately, input and output signal levels and limits were left as an excercise for the student to develop, with reference to the specified components - both explicitly named (SoC and Transceiver), and general (sensors needed, relay).
  ...
  TODO: block diagram of subcircuits

  With this visualization, it was apparent that the \acrshort{soc} took up the lion's share of connective complexity. It follows that the limitations and capabilities of the chip must be fully understood in order to .
  
  \subsection{Understanding the SoC}

  From the connection diagram and subsequent block diagram, it was determined the SoC is the core component, having the most connections with the greatest level of complexity. Thus gaining a strong
  
	

  With a better grasp of the problem at hand in terms of objectives, constraints, and good practice, a design cycle to produce a working electrical diagram can begin.
